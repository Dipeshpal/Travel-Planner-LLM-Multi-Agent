...
# Ensure Brave API key is set in environment for MCP
if brave_api_key:
    os.environ["BRAVE_API_KEY"] = brave_api_key
else:
    st.error("BRAVE_API_KEY not found in .env file. Please add it.")
    st.stop()
...
def parse_uploaded_file(uploaded_file):
    ...
    try:
        ...
        # Create a temporary file path for LangChain loaders
        # Note: LangChain loaders often expect file paths. This is a workaround.
        # A more robust solution might involve custom loaders or in-memory handling if available.
        ...
        if file_extension == ".csv":
            # Read CSV as pandas DataFrame and convert to markdown table
            ...
            # TextLoader can sometimes handle BytesIO directly, but path is safer
            loader = TextLoader(file_path=temp_file_path, encoding='utf-8') # Specify encoding
            documents = loader.load()
            content = "\n".join([doc.page_content for doc in documents])
        else:
        ...
        # Clean up temp file in case of error during parsing
...
def create_agents(api_key, include_flight_agent=True, include_hotel_agent=True):
    """Creates and returns the travel agents."""
    mcp_tool = MCP("npx -y @modelcontextprotocol/server-brave-search", env={"BRAVE_API_KEY": api_key})
    llm_model = "groq/deepseek-r1-distill-llama-70b" # Using a recommended Groq model
...
if generate_plan:
    if not destination or not dates or not budget or not preferences:
        ...
        try:
            # Determine existing plans content
...
"""
            ...
            # Create agents based on preferences
            ...
            # Make the result scrollable with custom CSS
            st.markdown("""
                <style>
                .scrollable-result {
                    max-height: 500px;
            ...
            with col5:
                def generate_image_from_text(text, img_filename):
                    # Basic image export: white background, black text
                    font_size = 16
                    font = None
                    try:
